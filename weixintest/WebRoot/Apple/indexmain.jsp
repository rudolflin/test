	...
	<script type="text/javascript">
	   ...
	   var appleNum=20;
		...
		$.ajax({
	        type: "post",
	        // 接口
	        url: "../Apple?openId="+openId+"&b="+Math.random(),
	        // data : 'ifGet',
	        dataType: "json",
	        success: function (data) {
				...
				var Names = data.Names;
					var myurl ="http://bigdata.chinaunicom.com/send/Apple/indexmain.jsp?openId="+openId;	
//	    alert(myurl);
				...
				var shareDataA=	 {
					...
					"desc" : "我已经收获了"+data.num+"颗苹果，有机会获得U盘、流量等好礼了，你也快来吧！",
					"link" : link,
				...
				// 加载页面后进行判断是不是已经摘过苹果，进行样式替换
				if(times == 1){
					$('.apple i').html('明日再来');
					$('.apple i').css('color','#bbb');
					$('.apple button').attr('disabled','disabled');
				}else{
					$('.apple button').on('click',function(){
						$.ajax({
					        type: "post",
					        // 接口
					        url: "../GetApple?openId="+openId+"&c="+Math.random(),
					        data : 'ifGet',
					        dataType: "json",
					        beforeSend:function(){
					        	$('.apple button').attr('disabled','disabled');
					        ...
					        success: function (data) {
					        		if(data.ok == 1){
										...
										$('.apple button').addClass('move');
										$('.baskapple em').addClass('item');
										setTimeout(function(){
											$('.apple button').removeClass('move');
											$('.apple button').addClass('active');
											$('.apple i').html('明日再来');
											$('.apple i').css('color','#bbb');
											$('.baskapple em').removeClass('item');
											// 增加一颗苹果
											$('.baskapple b').html(num+1+'颗');
											// 如果等于零说明前面已经初始化了篮子里面的苹果，需要重新进行装苹果的图片
											if(num == 0){
												$('.baskapple').css('background-image','url(img/applehave.png)');
											}
										}, 2000)
					        		}else{
					        			alert('姿势不对吧，在摘一次');
					        	...
					        	error : function(){
					        		$('.apple button').removeAttr('disabled');
					        		alert('网络不给力，再摘一次');
					        	}   
							});
						});
		...
		// 点击喊小伙伴来收苹果，进行蒙版提示进行分享
		$('section .fr').on('touchstart',function(){
			$('.mask').fadeIn('fast');
		});
		// 点击蒙版后，蒙版消失
		...
		$('section .fl').on('click',function(){
			window.location.href = "./indexnext.jsp?openId="+openId+"";
		});
		// 点击沃小树养育攻略进行规则页面的弹窗
		$('.fixed').on('touchstart',function(){
			
			$('.rule').css('visibility','visible');
			$('.rule').css('z-index','10002')
		});
	...
	.rule{height: 100%;width: 100%;position: fixed;left: 0;top: 0;right: 0;bottom: 0;background: url(./img/backtree.png) no-repeat center center;background-size: cover;z-index: 1001;visibility: hidden;z-index: -1;}
	.mask{position: fixed;left: 0;top: 0;right: 0;bottom: 0;background:rgba(0,0,0,0.8);background-size: 360px auto;display: none;z-index: 1000;text-align: center;}
	.mask img{width: 460px;margin-top: 50px;vertical-align: middle;}
	.mask span{display: inline-block;position: absolute;top: 252px;right:100px;font-size: 26px;color: #55B52F;line-height: 40px;}
	...
	header span{display: inline-block;width: 90px;height: 90px;border-radius: 50%;font-size: 0;border: 1px solid #8D8D8D;overflow: hidden;text-align: center;background: #fff;text-align: center;}
	header b{display: inline-block;width: 0;vertical-align: middle;height: 100%;}
	header span img{width: 92px;vertical-align: middle;border: none;border-width: 0;border-radius: 50%;height: 92px;position: relative;top: -1px;left: -1px;}
	...
	#wrapper{height: 636px;width: 500px;margin:15px auto 0 auto;position: relative;overflow: hidden;z-index: 1;}
	::-webkit-scrollbar {
	...
	#scroller{padding: 0 20px 0 20px;font-size: 22px;line-height: 30px;position: absolute;z-index: 1;left:0;top:0;-webkit-tap-highlight-color: rgba(0,0,0,0);-webkit-transform: translateZ(0);-moz-transform: translateZ(0);-ms-transform: translateZ(0);-o-transform: translateZ(0);transform: translateZ(0);-webkit-touch-callout: none;-webkit-user-select: none;-moz-user-select: none;-ms-user-select: none;user-select: none;-webkit-text-size-adjust: none;-moz-text-size-adjust: none;-ms-text-size-adjust: none;-o-text-size-adjust: none;text-size-adjust: none;}
...
<body onload="loaded()">
	...
	<!-- 头部 -->
		...
		<div class="tree clearfix">
			<div class="apple">
				<button class="active"><img src="img/apple.png" alt=""><i>摘苹果</i></button>
			</div>
			<!-- 篮子 水壶 小树的牌子-->
		...
		<section class="clearfix">
			<a class="fl">去苹果集市兑换好礼</a>
			<a class="fr">喊小伙伴来收苹果</a>
		</section>
		<!-- 底部分享二维码页面 -->
	...
	<div class="mask">
		<div>
			<img src="img/right_r2_c2.png" alt="">
			<span>点击右上通知小伙伴<br/>一起玩耍哟！</span>
		</div>
	</div>
	<!-- 规则点击页面 -->
